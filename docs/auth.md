...
### 12.1. Auto (background)

- Sync **только фоновый**: push на сервер запускается после autosave-debounce, без действий пользователя (autosync, larchanka-training/js-notebook#134).
- Ручного триггера нет: `SyncIndicator` — статус-индикатор (syncing / synced / offline / error), не кнопка.

### 12.1.1. Feature demo restore

`POST /api/v1/notebooks/features-demo/restore` восстанавливает канонический feature-demo notebook для текущего Bearer-пользователя.

Frontend вызывает endpoint только через `src/shared/api/notebook.ts` и показывает restore affordance на странице `/usage`.
...
| Save / autosave / sync              | **Нет**  | Автосейв и фоновый sync отключены на уровне store                                                             |
