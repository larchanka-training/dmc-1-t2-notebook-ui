...
LWW per-cell. Алгоритм описан в [API repo: docs/auth.md §8][api-auth]. Клиент **всегда** присылает полный массив `cells`, сервер делает merge.
...
Tombstones (мягкое удаление) — open question (см. [API repo: docs/auth.md §8.3][api-auth]). Возможный side-effect: ячейка может вернуться в ноутбук при синхронизации
