...
См. парный документ: `api/docs/auth.md`.
...
- `Resend in 45s` — UI-таймер (5 мин TTL OTP, после 45 сек разрешён повтор; запрос ограничен серверным rate limit, см. `api/docs/auth.md` §10).
...
```ts
export const accessTokenAtom = atom<string | null>(null, 'session.accessToken')
  .extend(withLocalStorage('session.accessToken'))

export const refreshTokenAtom = atom<string | null>(null, 'session.refreshToken')
  .extend(withLocalStorage('session.refreshToken'))

export const userAtom = atom<User | null>(null, 'session.user')
  .extend(withLocalStorage('session.user'))
```
...
frame-src 'self' blob:;     // для sandboxed iframe
object-src 'none';
base-uri 'self';
```
...
Слои 1–5 проектируются так, чтобы взлом одного не давал доступ к токенам. Но если ничего из этого не помогло — серверный refresh rotation (см. `api/docs/auth.md` §5.3) обнаружит reuse и отзовёт все сессии пользователя.
...
LWW per-cell. Алгоритм описан в `api/docs/auth.md` §8. Клиент **всегда** присылает полный массив `cells`, сервер делает merge.

**Клиентская часть:** при ответе сервера на PATCH (сервер может вернуть merged-результат, если поддерживается) — заменяем локальный state на серверный, перезаписываем IndexedDB. Если контракт PATCH возвращает только статус — делаем follow-up GET.
...
Tombstones (мягкое удаление) — open question (см. `api/docs/auth.md` §8.3). Возможный side-effect: ячейка может вернуться в ноутбук при синхронизации

---

## 13. Версионирование

Раздел собирает в одном месте всю информацию о версионировании на фронте: формат заметки, IndexedDB-схема, API, JWT. Парный раздел на бэке — `api/docs/auth.md` «Версионирование».

### 13.1. Что версионируется

| Сущность | Схема версионирования | Где подробности |
|---|---|---|
| Формат заметки (контракт данных) | `formatVersion: number` в каждой записи. Source of truth — бэк. | §13.2–13.5, `api/docs/auth.md` §9 |
...
Поведение: **read-only режим ноутбука**.

- Неизвестные `cell.kind` рендерятся как placeholder: `«Ячейка типа X не поддерживается этой версией приложения»`.
- Известные ячейки (`code`, `markdown`) рендерятся нормально, но без возможности редактировать весь ноутбук.
- Кнопки «Add cell», «Delete», «Run» для code-cells — доступны. Кнопки «Save» / autosave — блокированы. Autosave отключён.
- Наверху страницы — баннер: «Ноутбук в более новом формате (vN). Обновите страницу.» Кнопка «Reload» делает `location.reload()` с cache bypass.

Почему read-only, а не hard fail: пользователь видит свои данные и может их скопировать. Hard fail выглядел бы как «мой ноутбук пропал» — это неприемлемо.
...
Полный список версий формата — в `api/docs/auth.md` §9.6. На фронте не дублируем.
...
Баннер с OTP — **только** при наличии `otp` в ответе. Сервер защищён инвариантом «prod никогда не отдаёт otp» (см. `api/docs/auth.md` §6). Дополнительные клиентские проверки избыточны.
...
- **Tombstones для удалённых ячеек**: в v1 не реализуем. Возможны редкие «воскрешения» при multi-device sync.
