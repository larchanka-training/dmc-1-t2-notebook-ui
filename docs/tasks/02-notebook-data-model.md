...
- [x] `CellJSON`: `id` (UUID), `kind: 'code' | 'markdown'`, `content: string`,
      ...
      `toJSON`/`fromJSON`.
- [x] cell `id` — `crypto.randomUUID()` (бэк ждёт `format: uuid`).
...
- [x] CRUD в `src/shared/lib/storage/notebook.ts`: `get(id)`, `list()`,
      `put(notebook)`, `remove(id)`, `clear()`. Чтение прогоняется через
      `applyMigrations` + валидатор.
- [x] При первом запуске без ноутбука — seed «Welcome» (из `SEED_CODE`)
      сохраняется в store.

Отличие от плана: файл `storage/notebook.ts` (единственное число), метод
`remove` (не `delete` — зарезервированное слово). Отдельного `db.ts` нет —
`openDB` живёт в том же файле.
...
- [x] Сохраняем только при реальных изменениях — `dirtyAtom` хэширует
      сериализованные cells + title (по содержимому, не по ссылке).
- [x] Индикатор в шапке: `Saved · HH:MM:SS` / `Saving…` / `Save failed — retry`
      (`SaveIndicator`).
...
- Маршрут `/n/:id` и фоллбэк к API/MSW — **отложены** (single-notebook MVP,
  см. «Out of scope»). Загрузка асинхронная; отдельного skeleton пока нет
  (первый кадр — seed, затем подмена из БД).
...
src/features/notebook/
  ...
  model/
    ...
    autosave.ts                 ← debounce + dirtyAtom + saveStatusAtom
    autosave.test.ts
  ui/
    SaveIndicator.tsx           ← индикатор Saved/Saving/Failed
    SaveIndicator.test.tsx
src/shared/lib/storage/
  notebook.ts                   ← openDB + CRUD (get/list/put/remove/clear)
  notebook.test.ts
```
...
`notebook.ts` (без отдельного `db.ts`).
...
`notebookTitleAtom`, `notebookCreatedAtAtom`, плюс константа
`LOCAL_NOTEBOOK_ID` (single-notebook MVP). Ячейка несёт `updatedAt: Atom<number>`
