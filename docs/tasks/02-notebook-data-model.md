...
- [x] Overlapping saves внутри одной вкладки сериализованы: если пользователь
      ...
      а затем запускается второй save для новой dirty-версии. Baseline-хэш
      считается от реально записанного `NotebookJSON`, не от mutable state после
      `await`.
...
src/features/notebook/
  ...
  model/
    ...
    autosave.ts                 ← debounce + dirtyAtom + saveStatusAtom + cross-tab guard
    autosave.test.ts
  ui/
    SaveIndicator.tsx           ← индикатор Saved/Saving/Failed/Conflict
