...
- [x] CRUD в `src/features/notebook/persistence/storage.ts`: `get(id)`,
      `list()`, `put(notebook)`, `remove(id)`, `clear()`. Чтение прогоняется
      через `applyMigrations` + валидатор.
...
src/features/notebook/
  ...
  persistence/
    ...
    migrations.test.ts
    storage.ts                  ← openDB + CRUD (get/list/put/remove/clear)
    storage.test.ts
  model/
    notebook.ts                 ← cellsAtom + meta-атомы + loadNotebook + snapshot
                                  + notebookLoadedAtom (boot-gate)
