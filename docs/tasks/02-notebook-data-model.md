...
- [x] Схема БД (`DB_VERSION = 2`): object store `notebooks` (key = `id`,
      индекс по `updatedAt`) **плюс** store `sync` (key = `notebookId`) —
      партиция sync-метаданных (#134). Миграция `v1 → v2` **аддитивная** и
      версионно-защищённая (`oldVersion`-гарды): `notebooks` не пересоздаётся,
      существующий локальный ноутбук переживает бамп нетронутым (есть отдельный
...
- [x] CRUD в `src/features/notebook/persistence/storage.ts`: `get(id)`,
      ...
      валидатор.
- [x] **Storage-adapter seam (#133).** Эти низкоуровневые функции спрятаны за
      контрактом `NotebookStorageAdapter` (`storageAdapter.ts`, владеет также
      `PutResult` и общим правилом конфликта `isStaleWrite`). Реализаций две:
      `indexedDbAdapter` (диск, дефолт) и `createMemoryAdapter()` (в памяти, под
      untrusted-device режим #136). Autosave и boot-load ходят в активный backend
      через фасад `notebookStorage` (`activeStorage.ts`), а не напрямую в
      `storage.ts`. Дефолт и пользовательское поведение не меняются — IndexedDB
      остаётся единственным активным backend'ом; `clearLocalNotebookData()` —
      точка очистки под будущий sign-out: сегодня чистит только активный backend
      (для IndexedDB-дефолта верно), авторитетный all-local wipe (диск + очередь + `deletedCells`) и cross-tab под device-mode — за #136.
- [x] **Sync-партиция (#134).** Очередь несинхронизированных изменений (флаг
      `dirty` + `remoteCreated`) и буфер `deletedCells` (надгробия) на каждый
      ноутбук живут в отдельной именованной партиции **того же** активного
      backend'а — методы `getSyncState`/`putSyncState`/`deleteSyncState` на
      контракте `NotebookStorageAdapter`. `clearAll()` стирает обе партиции
      одной транзакцией, поэтому `clearLocalNotebookData()` ничего не оставляет.
      Режим хранения (диск/память) сам подстроится под доверенность устройства в
      #136. Сам фоновый синк описан в
      [`architecture/remote-sync.md`](../architecture/remote-sync.md).
...
src/features/notebook/
  ...
  persistence/
    ...
    migrations.test.ts
    storage.ts                  ← openDB + low-level CRUD + atomic putIfNewer CAS
    storage.test.ts
    storageAdapter.ts           ← NotebookStorageAdapter contract + PutResult + isStaleWrite (#133)
    storageAdapter.test.ts
    indexedDbAdapter.ts         ← disk adapter over storage.ts (active default) (#133)
    indexedDbAdapter.test.ts    ← disk-specific: NewerFormatError rethrow
    memoryAdapter.ts            ← ephemeral in-memory adapter for #136 (#133)
    memoryAdapter.test.ts       ← memory-specific: volatility + no browser-storage writes
    activeStorage.ts            ← active facade (notebookStorage) + clearLocalNotebookData (#133)
    activeStorage.test.ts
    adapterContract.test.ts     ← shared disk↔memory contract suite (#133)
    __fixtures__/notebook.ts    ← shared persistence test fixture (#133)
...
НЕТ (см. «Reatom-модель»); валидатор ручной, не Zod; IndexedDB-доступ — один
файл `persistence/storage.ts` (без отдельного `db.ts`), с #133 спрятанный за
контрактом `NotebookStorageAdapter` и активным фасадом `notebookStorage`
(`activeStorage.ts`); всё внутри notebook-слайса (не в `shared/lib` — это
фич-код, зависящий от доменных schema/migrations).
