...
- [x] Тема следует за глобальной (light/dark, из `entities/theme`, через CM Compartment).
...
- [x] `Esc` — выйти из режима редактирования в «command mode».
...
- [x] Drop-зоны подсвечиваются между ячейками.
...
- [x] Подсвечивает совпадения, навигация `Enter` / `Shift+Enter`. (**Факт:** активный match скроллит ячейку в вид; ин-ячеечная подсветка внутри CM — последующее улучшение.)
